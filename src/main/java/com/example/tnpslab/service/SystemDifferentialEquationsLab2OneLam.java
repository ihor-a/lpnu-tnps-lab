...
public class SystemDifferentialEquationsLab2OneLam implements SystemDifferentialEquations {
    ...
    public double[] derivn(double t, double[] p) {
        ...
        dpdt[8] = -p[8]*(l1+l2+l3+l4+l5)+p[13]*m1+p[14]*m2+p[18]*m3+p[23]*m4+p[28]*m5;
        ...
        dpdt[13] = -p[13]*(l2+l3+l4+l5+m1)+p[8]*l1+p[33]*m2+p[60]*m3+p[37]*m4+p[44]*m5 +p[10]*m2+p[16]*m3;
        dpdt[14] = -p[14]*(l1+l3+l4+l5+m2)+p[8]*l2+p[33]*m1+p[74]*m3+p[63]*m4+p[58]*m5 +p[12]*m3;
        dpdt[15] = -p[15]*(m1+m3)+p[3]*l1+p[1]*l3;
        dpdt[16] = -p[16]*m3+p[1]*l3;
        dpdt[17] = -p[17]*(m4+m5)+p[26]*l4+p[21]*l5;
        dpdt[18] = -p[18]*(l1+l2+l4+l5+m3)+p[8]*l3+p[60]*m1+p[74]*m2+p[52]*m4+p[55]*m5 +p[12]*m2;
        ...
        dpdt[23] = -p[23]*(l1+l2+l3+l5+m4)+p[8]*l4+p[37]*m1+p[63]*m2+p[52]*m3+p[62]*m5 +p[22]*m2+p[25]*m3;
        ...
        dpdt[28] = -p[28]*(l1+l2+l3+l4+m5)+p[8]*l5+p[44]*m1+p[58]*m2+p[55]*m3+p[62]*m4 +p[27]*m2+p[30]*m3;
        ...
        dpdt[33] = -p[33]*(l3+l4+l5+m1+m2)+p[14]*l1+p[13]*l2+p[59]*m3+p[61]*m4+p[49]*m5 +p[32]*m3;
        ...
        dpdt[37] = -p[37]*(l2+l3+l5+m1+m4)+p[23]*l1+p[13]*l4+p[61]*m2+p[70]*m3+p[65]*m5 +p[36]*m2+p[41]*m3;
        ...
        dpdt[44] = -p[44]*(l2+l3+l4+m1+m5)+p[28]*l1+p[13]*l5+p[49]*m2+p[64]*m3+p[65]*m4 +p[43]*m2+p[46]*m3;
        dpdt[45] = -p[45]*(m1+m3)+p[29]*l1+p[20]*l3;
        dpdt[46] = -p[46]*m3+p[20]*l3;
        dpdt[47] = -p[47]*m3+p[42]*l3;
        dpdt[48] = -p[48]*m3+p[42]*l3;
        dpdt[49] = -p[49]*(l3+l4+m1+m2+m5)+p[58]*l1+p[44]*l2+p[33]*l5+p[73]*m3+p[72]*m4 +p[48]*m3;
        dpdt[50] = -p[50]*(m2+m3)+p[24]*l2+p[21]*l3;
        dpdt[51] = -p[51]*(m2+m3)+p[24]*l2+p[21]*l3;
        dpdt[52] = -p[52]*(l1+l2+l5+m3+m4)+p[23]*l3+p[18]*l4+p[70]*m1+p[69]*m2+p[68]*m5 +p[51]*m2;
        dpdt[53] = -p[53]*(m2+m3)+p[29]*l2+p[26]*l3;
        dpdt[54] = -p[54]*(m2+m3)+p[29]*l2+p[26]*l3;
        dpdt[55] = -p[55]*(l1+l2+l4+m3+m5)+p[28]*l3+p[18]*l5+p[64]*m1+p[66]*m2+p[68]*m4 +p[54]*m2;
        dpdt[56] = -p[56]*(m4+m5)+p[29]*l4+p[24]*l5;
        dpdt[57] = -p[57]*(m4+m5)+p[20]*l4+p[19]*l5;
        dpdt[58] = -p[58]*(l1+l3+l4+m2+m5)+p[28]*l2+p[14]*l5+p[49]*m1+p[66]*m3+p[67]*m4 +p[54]*m3;
        dpdt[59] = -p[59]*m3+p[33]*l3;
        dpdt[60] = -p[60]*(m1+m3)+p[18]*l1+p[13]*l3;
        dpdt[61] = -p[61]*(l3+l5+m1+m2+m4)+p[63]*l1+p[37]*l2+p[33]*l4+p[71]*m3+p[72]*m5 +p[75]*m3;
        dpdt[62] = -p[62]*(m4+m5)+p[28]*l4+p[23]*l5;
        dpdt[63] = -p[63]*(l1+2*l3+l5+m2+m4)+p[23]*l2+p[14]*l4+p[61]*m1+p[69]*m3+p[67]*m5 +p[51]*m3;
        dpdt[64] = -p[64]*(m1+m3)+p[55]*l1+p[44]*l3;
        dpdt[65] = -p[65]*(m4+m5)+p[44]*l4+p[37]*l5;
        dpdt[66] = -p[66]*(m2+m3)+p[55]*l2+p[58]*l3;
        dpdt[67] = -p[67]*(m4+m5)+p[58]*l4+p[63]*l5;
        dpdt[68] = -p[68]*(m4+m5)+p[55]*l4+p[52]*l5;
        dpdt[69] = -p[69]*(m2+m3)+p[52]*l2+p[63]*l3;
        dpdt[70] = -p[70]*(m1+m3)+p[52]*l1+p[37]*l3;
        dpdt[71] = -p[71]*m3+p[61]*l3;
        dpdt[72] = -p[72]*(m4+m5)+p[49]*l4+p[61]*l5;
        dpdt[73] = -p[73]*m3+p[49]*2*l3;
        dpdt[74] = -p[74]*(m2+m3)+p[18]*l2+p[14]*l3;
